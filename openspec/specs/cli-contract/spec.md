...
Every operational command other than `dbox activity export --format jsonl` SHALL
write exactly one valid JSON value to stdout on success, without headers,
diagnostics, or auxiliary text. `dbox activity export` SHALL produce its
portable data on stdout: its default `json` format is exactly one JSON array,
while its `jsonl` format is one valid JSON object per line and no auxiliary
output. The CLI SHALL NOT accept `--output`, and JSON output for commands other
than the documented export format SHALL NOT require a format-selection option.
...
errors, `2` for validation or syntax errors, `3` for missing resources or
request conflicts, and `4` for missing databases, migration failures, I/O
failures, or other database errors. A stale expected version or an import ID
collision SHALL return `conflict_error` with exit code `3`; failure to open or
read an import file SHALL return `io_error` with exit code `4`.
